...
export default class Terrain {
  ...
  initTerrainGeometry() {
    const geometry = new PlaneGeometry(this.size, this.size, this.size, this.size);
    ...
    const uniforms = {
      map: { type: "t", value: this.textures[1] },
      scale: { type: "f", value: 10 },
      phongMaterial: {
        value: {
  ...
  createGhostPlane(translation) {
    ...
    const ghostPlaneGeo = new PlaneGeometry(this.size, this.size, this.size, this.size);
